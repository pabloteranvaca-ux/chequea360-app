...
with col_text:
    header_html = (
        ...
        '<div class="header-subtitle">Plataforma de inteligencia informativa impulsada por Ecuador Chequea y ChequeaLab.</div>'
...
Ejemplos: Ecuador pobreza · Compara desempleo entre Ecuador y Colombia · ¿Cuál es la inflación en América Latina? · What is unemployment in South America? · Qual é a inflação na América Latina?
</span>
</div>
""", unsafe_allow_html=True)
...
LATIN_AMERICA = [
    ...
    "El Salvador", "Guatemala", "Guyana", "Haití", "Honduras",
    "Jamaica", "México", "Nicaragua", "Panamá", "Paraguay",
    "Perú", "Puerto Rico", "Surinam", "Trinidad y Tobago",
    "Uruguay", "Venezuela"
]
...
AMERICA = [
    ...
    "Panamá", "Paraguay", "Perú", "Puerto Rico", "Surinam",
...
def labels(lang):
    ...
    if lang == "pt":
        return {
            ...
            "source_word": "Fonte",
            "indicator_word": "Indicador",
            "code_word": "Código",
            "countries_word": "Países"
        }
        ...
        if not all_data:
            ...
            with col2:
                ...
                trace_html = (
                    ...
                    f'<h4>🔎 {L["source"]}</h4>'
                    f'<p><strong>{L["source_word"]}:</strong> Banco Mundial</p>'
                    f'<p><strong>{L["indicator_word"]}:</strong> {selected_indicator["name"]}</p>'
                    f'<p><strong>{L["code_word"]}:</strong> {selected_indicator["code"]}</p>'
                    f'<p><strong>{L["countries_word"]}:</strong> {countries_text}</p>'
...
st.markdown(
    '<div class="footer">Chequea360 · Ecuador Chequea · ChequeaLab · Datos oficiales del Banco Mundial</div>',
    unsafe_allow_html=True
)